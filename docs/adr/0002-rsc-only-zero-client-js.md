...
The landing page is a single-route, static, marketing surface. SPEC
NFR-06 sets the budget at "initial JS payload (transferred) < 100 KB"
and "no client-side framework hydration for above-the-fold content."
NFR-01 sets the Core Web Vitals targets (LCP < 2.0 s, INP < 200 ms,
CLS < 0.05).
...
3. **Mostly client-side.** A few RSCs for SEO, but most rendering
   happens in the browser. Best for highly interactive apps.

The landing has **no** required client-side interactivity in v1: the
CTA email form is a deliberate stub (submit is `disabled`), and the
icons are static. The page reads as a document, not an app.

The build-landing deliverable verified zero `"use client"` directives
...
**Every component in the landing is a React Server Component. Zero
`"use client"` directives anywhere in the tree.** Future client-side
interactivity (consent banner, opt-in analytics toggle, the real
email-capture Server Action) is added as a thin `"use client"` island
only when the feature's UX requires it, and only after a Lighthouse
re-run proves the budget is intact.
...
explicitly out of scope per the build-landing task's "do not install
...
- The page works without JavaScript at all, which is a meaningful
...
- The same "no client JS" posture will not transfer to the future
  docs site, which may have search, syntax highlighting, and
  interactive examples. That project gets its own ADR.
